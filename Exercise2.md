...
2. Select **Bash** if prompted to start the **cloud shell**.
...
1. In the Azure portal, navigate to your **resource group**, and click on your newly-created **Virtual Machine Scale Set**. Refresh your azure portal if you don't see it in your resource group. 

2. Go to the **Instances**. Note that you only have one instance, as the **ARM template** you deployed had this configuration.

   ![](images/scalesetinstances.png)
   
3. Go to the individual instance. Here you can view details for your individual VMSS instance and you can restart, deallocate, reimage, or upgrade the instances if needed.
...
**Note: What is upgrading?**
   ...
   * In the instance view, you can also access features like Azure Bastion(secure way to connect to your virtual machines), Serial console, and Boot diagnostics.
...
5. Go to the **Instances** and watch VM getting created. Note that it might take a few minutes for this to reflect this.
...
4. Click on the **Add a scale set condition**.
   ...
   - **Start date**: Enter a time of 2 minutes from now to test the scheduler.
   - **End date**: Enter a time of 10 minutes from now to test the scheduler.
...
4. Update the VMSS model to enable **Boot Diagnostics**, click **Save**.
